...
export function accountErrorMessage(code: AccountErrorCode) {
    switch (code) {
        ...
        case "invalid_otp_length": return "Enter exactly six digits. If your email contains a different number, request a fresh code."
        case "invalid_or_expired_otp": return "That code was not accepted. It may be incorrect, expired, or from an older email; request a fresh code and use all six digits."
...
export function classifyAccountError(error: unknown): AccountErrorCode {
    ...
    if (message.includes("expired") || message.includes("invalid otp")) return "invalid_or_expired_otp"
