...
- `AnchoredPopup` defaults to above-trigger placement. A dense feature editor may explicitly request below-trigger placement when its trigger uses disclosure-chevron semantics and the editor needs a controlled, internally scrolling height; list actions and detail-field popups retain their documented above-trigger placement.
