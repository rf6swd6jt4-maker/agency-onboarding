-- The first client-portal attachment projection accidentally selected Vault's
-- encrypted storage value (`secret`) instead of its usable plaintext value
-- (`decrypted_secret`). Replace it without changing the public signature.
    ...
    with decoded as materialized (
        select
            ...
            case
                ...
                else communications_secure.try_decrypt_jsonb(message.raw_payload_ciphertext, content_secret.decrypted_secret)
