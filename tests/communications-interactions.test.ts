...
test("message interactions keep the approved mobile and profile parity", async () => {
    ...
    assert.match(jumpToLatest, /export \{ observeMessagePaneResize \}/)
    assert.match(messagePaneScroll, /new ResizeObserver/)
    assert.match(messagePaneScroll, /pane\.addEventListener\("scroll", rememberScrollPosition/)
    assert.match(messagePaneScroll, /if \(pane\.clientHeight !== previousHeight\)[\s\S]*scheduleAnchor\(\)/)
    assert.doesNotMatch(messagePaneScroll, /rememberScrollPosition = \(\) => \{\s*previousHeight = pane\.clientHeight/)
    assert.match(messagePaneScroll, /window\.requestAnimationFrame\(applyAnchor\)/)
    assert.match(messagePaneScroll, /if \(layoutTarget\) observer\.observe\(layoutTarget\)/)
    assert.match(messagePaneScroll, /pane\.scrollTo\(\{ top: nextScrollTop, left: 0 \}\)/)
    assert.doesNotMatch(messagePaneScroll, /composerHeightDelta|paneHeightDelta|observer\.observe\(composer\)/)
