...
test("message interactions keep the approved mobile and profile parity", async () => {
    ...
    assert.doesNotMatch(shell, /visualViewport\?\.(?:add|remove)EventListener\("scroll", holdWorkspaceViewport\)/)
    ...
    assert.match(jumpToLatest, /const heightDelta = previousHeight - nextHeight/)
    assert.match(jumpToLatest, /if \(preserveVisibleBottom\) pane\.scrollTo\(\{ top: pane\.scrollTop \+ heightDelta, left: 0 \}\)/)
    assert.match(jumpToLatest, /else if \(isFollowingLatest\(\)\) pane\.scrollTo/)
