...
test("message interactions keep the approved mobile and profile parity", async () => {
    ...
    assert.match(jumpToLatest, /if \(followingLatest\) pane\.scrollTo\(\{ top: pane\.scrollHeight, left: 0 \}\)/)
    assert.match(jumpToLatest, /else if \(preserveVisibleBottom\) pane\.scrollTo\(\{ top: pane\.scrollTop \+ heightDelta, left: 0 \}\)/)
