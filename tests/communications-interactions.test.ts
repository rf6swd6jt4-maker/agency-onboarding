...
test("message interactions keep the approved mobile and profile parity", async () => {
    ...
    assert.match(panel, /viewport\?\.height \?\? window\.innerHeight/)
    assert.doesNotMatch(panel, /offsetTop|panel\.style\.transform/)
    ...
    assert.doesNotMatch(shell, /--workspace-visual-viewport-top|visualViewport\?\.offsetTop/)
    assert.match(globals, /html\[data-workspace-viewport-locked="true"\]/)
    assert.match(globals, /position: fixed;/)
    assert.doesNotMatch(globals, /\[data-workspace-topbar\].*transform|workspace-visual-viewport-top/s)
    assert.match(globals, /height: calc\(var\(--workspace-visual-viewport-height, 100dvh\) - 6\.25rem\)/)
    assert.doesNotMatch(globals, /communications-keyboard-inset/)
