...
test("message interactions keep the approved mobile and profile parity", async () => {
    ...
    assert.match(team, /<MessageDeleteConfirmation/)
    assert.match(team, /finishMessageSwipe/)
    const deleteConfirmation = await readFile("components/communications/MessageDeleteConfirmation.tsx", "utf8")
    assert.match(deleteConfirmation, /dialog\.showModal\(\)/)
    assert.match(deleteConfirmation, /cannot be undone/)
    assert.match(deleteConfirmation, /onClick=\{onConfirm\}/)
    ...
    assert.match(paneInteractions, /if \(gesture\.moved\) markUserScroll\(\)\s+else if .*event\.target\.closest\("button,a,input,textarea,select,video,audio,.*composerRef\.current\?\.blur\(\)/)
