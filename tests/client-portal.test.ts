...
test("the portal message bridge is token scoped and strips staff-only communication data", () => {
    ...
    assert.match(portalMessages, /supabaseAdmin\.rpc\("client_portal_messages"/u)
    ...
    assert.doesNotMatch(portalMessagesRoute, /export async function DELETE/u)
})

test("the client portal renders one branded, deliberately simple relationship chat", () => {
    ...
    assert.doesNotMatch(portalChat, /Delete message/u)
    assert.doesNotMatch(portalChat, /reaction/u)
})
