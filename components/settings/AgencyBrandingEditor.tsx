...
export function AgencyBrandingEditor({ workspaceSlug, workspaceName, initialTheme, publishedTheme: initialPublishedTheme, previewBookend, help, schemaReady, faviconSrc, uploadFavicon }: { workspaceSlug: string; workspaceName: string; initialTheme: OnboardingThemeDefinition; publishedTheme: OnboardingThemeDefinition; previewBookend: OnboardingBookendDefinition; help: OnboardingHelpSettings; schemaReady: boolean; faviconSrc: string | null; uploadFavicon: (formData: FormData) => Promise<void> }) {
    ...
    return <div className="min-w-0 max-w-full space-y-5">
        ...
        {previewOpen && modalTarget ? createPortal(<div data-agency-branding-preview className="fixed inset-0 z-[2147483646] overflow-hidden bg-black text-white">
            <div className="absolute inset-x-0 top-0 z-20 flex items-start justify-between gap-3 p-3 sm:p-4">
                <button type="button" onClick={() => { setPreviewOpen(false); setPublishReviewOpen(false) }} className="rounded-full border border-white/20 bg-black/75 px-4 py-2 text-xs font-semibold text-white shadow-lg backdrop-blur transition hover:bg-black focus:outline-none focus:ring-2 focus:ring-white/70">Exit preview</button>
                <button type="button" disabled={!schemaReady || publishPending || !colourChanges.length} onClick={() => { setError(null); setPublishReviewOpen(true) }} className="rounded-full bg-white px-4 py-2 text-xs font-semibold text-black shadow-lg transition hover:bg-neutral-100 disabled:cursor-default disabled:opacity-45">{colourChanges.length ? "Publish" : "Published"}</button>
            </div>
            ...
            {publishReviewOpen ? <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/75 p-3 backdrop-blur-sm sm:p-4" onMouseDown={(event) => { if (event.target === event.currentTarget && !publishPending) setPublishReviewOpen(false) }}>
                <section role="dialog" aria-modal="true" aria-labelledby="publish-colour-title" className="flex max-h-[min(90dvh,42rem)] w-full max-w-xl flex-col overflow-hidden rounded-2xl border border-neutral-700 bg-neutral-950 shadow-2xl shadow-black/70">
                    ...
                    <div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-4">
                        <div className="grid grid-cols-[minmax(0,1fr)_2rem_minmax(0,1fr)] gap-2 px-2 pb-2 text-[10px] font-medium uppercase tracking-wide text-neutral-600"><span>Published</span><span /><span>Draft</span></div>
                        <div className="space-y-2">{colourChanges.map((change) => <div key={change.slot} className="rounded-xl border border-neutral-800 bg-black p-3"><p className="mb-2 text-xs font-medium text-neutral-400">{ONBOARDING_THEME_SLOT_LABELS[change.slot]}</p><div className="grid grid-cols-[minmax(0,1fr)_2rem_minmax(0,1fr)] items-center gap-2"><span className="flex min-w-0 items-center gap-2"><span aria-hidden="true" className="h-7 w-7 shrink-0 rounded-md border border-white/10" style={{ backgroundColor: change.current.hex }} /><span className="min-w-0"><span className="block truncate text-xs text-neutral-200">{change.current.name}</span><span className="block truncate font-mono text-[10px] text-neutral-600">{change.current.hex}</span></span></span><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-4 w-4 justify-self-center text-neutral-600"><path d="M5 12h14M14 7l5 5-5 5" /></svg><span className="flex min-w-0 items-center gap-2"><span aria-hidden="true" className="h-7 w-7 shrink-0 rounded-md border border-white/10" style={{ backgroundColor: change.next.hex }} /><span className="min-w-0"><span className="block truncate text-xs text-neutral-200">{change.next.name}</span><span className="block truncate font-mono text-[10px] text-neutral-600">{change.next.hex}</span></span></span></div></div>)}</div>
                    </div>
