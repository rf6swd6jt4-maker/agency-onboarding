...
export function TrendChart({
    ...
    return <div className="min-w-0">
        <svg viewBox={`0 0 ${chartWidth} 232`} className="aspect-[2.4/1] w-full overflow-visible" role="img" aria-label={ariaLabel} onPointerLeave={() => setActiveId(null)}>
            ...
            {activePoint ? <g pointerEvents="none"><circle cx={x(activePoint.position)} cy={y(activePoint.value)} r="4" fill="white" stroke="black" strokeWidth="2" /><rect x={tooltipX} y={tooltipY} width="168" height="44" rx="7" fill="rgb(23 23 23)" stroke="rgb(82 82 82)" /><text x={tooltipX + 9} y={tooltipY + 17} fill="rgb(163 163 163)" className="text-[12px] sm:text-[9px]">{activePoint.tooltipLabel}</text><text x={tooltipX + 9} y={tooltipY + 35} fill="white" fontWeight="600" className="text-[14px] sm:text-[10px]">{activePoint.tooltipValue}</text></g> : null}
