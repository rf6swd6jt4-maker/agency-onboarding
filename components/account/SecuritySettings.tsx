...
export function SecuritySettings({ email, initialFactors }: { email: string; initialFactors: Factor[] }) {
    ...
    return <div className="space-y-6">
        ...
        <section className="rounded-2xl border border-neutral-800 bg-neutral-900 p-5">
            ...
            {pendingRemoval ? <div role="dialog" aria-label="Confirm authenticator removal" className="mt-5 rounded-xl border border-yellow-900/70 bg-yellow-950/20 p-4"><Status label="Confirmation required" tone="yellow" /><p className="mt-3 text-sm leading-6 text-neutral-200">Remove <span className="font-medium text-white">{pendingRemoval.friendlyName}</span>? Codes from it will stop working immediately. Your remaining authenticator will continue protecting the account.</p><div className="mt-4 flex flex-wrap gap-2"><button type="button" disabled={removing} onClick={() => void removeFactor(pendingRemoval.id)} className="min-h-10 rounded-lg bg-red-100 px-3 py-2 text-sm font-semibold text-red-950 disabled:opacity-50">{removing ? "Removing…" : "Remove authenticator"}</button><button type="button" disabled={removing} onClick={() => setPendingRemoval(null)} className="min-h-10 rounded-lg border border-neutral-700 px-3 py-2 text-sm disabled:opacity-50">Keep it</button></div></div> : null}
