...
function TrendChart({ result, days }: { result: OkrKeyResult; days: OkrReportingDay<OkrMeasurement>[] }) {
    ...
    const { min, max, showZero } = okrTrendScale({ baseline: result.baseline_value, target: result.target_value, values: series.map((point) => point.value), comparator: result.comparator })
...
export function OkrWorkspace({ workspaceSlug, currentUserId, okrs, ownerOptions, workItems, people, today }: Props) {
    ...
    return <div className="mt-5">
        ...
        {dialog?.type === "add-work" && selectedOkr && selectedResult ? (() => {
            ...
            return <Modal title="Add work" description={`Create or link work for ${selectedResult.name}.`} error={error} onClose={() => showDialog(returnTo)}><div className="grid gap-5 p-4 sm:p-5 md:grid-cols-2"><form onSubmit={(event) => run(event, createOkrAction.bind(null, workspaceSlug, selectedOkr.id, selectedResult.id), returnTo)}><h3 className="font-medium">Create work item</h3><label className="mt-3 block text-sm text-neutral-300">Title<input name="title" required autoFocus placeholder="What needs to happen?" className={modalInputClass} /></label><label className="mt-3 block text-sm text-neutral-300">Description <span className="text-neutral-600">(optional)</span><textarea name="description" rows={3} className={modalTextareaClass} /></label><OkrWorkEstimateFields result={selectedResult} /><button disabled={pending} className="mt-4 h-10 w-full rounded-md bg-white px-3 text-sm font-medium text-black disabled:opacity-50">Create and link</button></form><form onSubmit={(event) => run(event, linkOkrAction.bind(null, workspaceSlug, selectedOkr.id, selectedResult.id), returnTo)} className="border-t border-neutral-800 pt-5 md:border-l md:border-t-0 md:pl-5 md:pt-0"><h3 className="font-medium">Link existing work</h3>{available.length ? <><label className="mt-3 block text-sm text-neutral-300">Work item<select name="work_item_id" className={modalInputClass}>{available.map((item) => <option key={item.id} value={item.id}>{item.title} · {workItemPriorityLabel(item.priority)}</option>)}</select></label><OkrWorkEstimateFields result={selectedResult} /><button disabled={pending} className="mt-4 h-10 w-full rounded-md border border-neutral-700 px-3 text-sm text-neutral-200 disabled:opacity-50">Link work item</button></> : <p className="mt-3 text-sm leading-6 text-neutral-600">Every available work item is already linked to this Key Result.</p>}</form></div></Modal>
        })() : null}
    </div>
}
