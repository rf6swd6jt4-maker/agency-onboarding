                ...
                {selected ? <>
                    ...
                    <footer className="shrink-0 border-t border-neutral-800 bg-neutral-950 p-3 sm:p-4">
                        ...
                        <div className="mx-auto flex max-w-3xl items-center gap-2 rounded-xl border border-neutral-800 bg-black px-3 py-2 focus-within:border-neutral-600">
                            ...
                            <div className="relative min-w-0 flex-1">{!draft ? <span aria-hidden="true" className="pointer-events-none absolute inset-x-0 top-2 truncate text-sm leading-5 text-neutral-600">{selected.canSend ? `Message ${selected.title}` : "Add a WhatsApp number to this relationship"}</span> : null}<textarea ref={composerRef} rows={1} value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void sendMessage() } }} disabled={!bootstrap.schemaReady || !selected.canSend} aria-label={`Message ${selected.title}`} className="relative max-h-28 min-h-9 w-full resize-none bg-transparent py-2 text-sm leading-5 outline-none disabled:cursor-not-allowed" /></div>
