            ...
            <div className={`${selected ? "flex" : "hidden lg:flex"} min-h-0 min-w-0 flex-col bg-black`}>
                {selected ? <>
                    <header className="flex h-14 shrink-0 items-center gap-3 border-b border-neutral-800 bg-neutral-950 px-3 sm:px-4">
                        ...
                        <button type="button" onClick={() => selected.kind === "direct" ? openWorkspaceMemberProfile(selected.memberIds.find((id) => id !== bootstrap.currentUser.id) ?? bootstrap.currentUser.id) : currentTeam ? setEditingTeam(currentTeam) : undefined} aria-label={selected.kind === "direct" ? `Open ${selected.title} profile` : `Open ${selected.title} settings`} className="flex min-w-0 flex-1 cursor-pointer items-center gap-3 rounded-lg text-left outline-none hover:text-neutral-200 focus-visible:ring-2 focus-visible:ring-neutral-600">
                    ...
                    <div ref={messagePaneRef} className="min-h-0 flex-1 overflow-y-auto overscroll-contain bg-[radial-gradient(circle_at_top,_rgba(38,38,38,0.5),_transparent_38%)] px-3 py-5 sm:px-6"><div className="mx-auto flex max-w-3xl flex-col gap-2">{selected.messages.length ? selected.messages.map((message, index) => {
                        ...
                        return <Fragment key={message.id}>
                            ...
                            <div data-message-interaction={message.id} className={`relative flex items-end ${own ? "justify-end" : "justify-start"}`}>
                                ...
                                <span aria-hidden="true" style={{ top: swipePosition?.id === message.id && swipePosition.actionTop !== undefined ? swipePosition.actionTop : "50%", opacity: Math.min(1, Math.max(0, swipeOffset) / 38), transform: `translateY(-50%) scale(${0.72 + Math.min(0.28, Math.max(0, swipeOffset) / 190)})` }} className="pointer-events-none absolute left-0 flex h-9 w-9 items-center justify-center rounded-full bg-neutral-800 text-white lg:hidden"><ReplyIcon className="h-5 w-5" /></span>
                                {canDelete ? <span aria-hidden="true" style={{ top: swipePosition?.id === message.id && swipePosition.actionTop !== undefined ? swipePosition.actionTop : "50%", opacity: Math.min(1, Math.max(0, -swipeOffset) / 38), transform: `translateY(-50%) scale(${0.72 + Math.min(0.28, Math.max(0, -swipeOffset) / 190)})` }} className="pointer-events-none absolute right-0 flex h-9 w-9 items-center justify-center rounded-full bg-red-600 text-white lg:hidden"><DeleteIcon className="h-5 w-5" /></span> : null}
                                ...
                                <article
                                    ...
                                    onTouchStart={(event) => { const touch = event.touches[0]; swipeStartRef.current = touch ? { id: message.id, x: touch.clientX, y: touch.clientY, cancelled: false } : null; if (touch) { const messageRect = event.currentTarget.getBoundingClientRect(); const viewportRect = messagePaneRef.current?.getBoundingClientRect() ?? { top: 0, bottom: window.innerHeight }; setSwipePosition({ id: message.id, offset: 0, active: true, actionTop: visibleSwipeActionTop(messageRect, viewportRect) }) } }}
                                    onTouchMove={(event) => { const start = swipeStartRef.current; const touch = event.touches[0]; if (!start || start.id !== message.id || !touch || start.cancelled) return; const deltaX = touch.clientX - start.x; const deltaY = touch.clientY - start.y; if (Math.abs(deltaY) > Math.abs(deltaX) && Math.abs(deltaY) > 8) { start.cancelled = true; setSwipePosition((current) => ({ id: message.id, offset: 0, active: false, actionTop: current?.id === message.id ? current.actionTop : undefined })); return } const constrained = canDelete ? Math.max(-82, Math.min(82, deltaX * 0.78)) : Math.max(0, Math.min(82, deltaX * 0.78)); if (Math.abs(constrained) > 2) { event.preventDefault(); setSwipePosition((current) => ({ id: message.id, offset: constrained, active: true, actionTop: current?.id === message.id ? current.actionTop : undefined })) } }}
                                ...
                                >
                                    {selected.kind === "team" ? <button type="button" onClick={(event) => { event.stopPropagation(); openWorkspaceMemberProfile(message.senderUserId) }} className="mb-1 text-[10px] font-semibold text-neutral-500 hover:underline">{own ? "You" : sender?.name ?? "Team member"}</button> : null}
                                    ...
                                    {message.attachment ? <NativeAttachment attachment={message.attachment} onOpenImage={setPreviewMedia} /> : null}
                                    ...
                                    <div className={`mt-1.5 flex items-center justify-between gap-3 text-[10px] ${own ? "text-neutral-500" : "text-neutral-600"}`}>
                                        <span className="flex min-w-0 -space-x-1">{selected.kind === "team" ? readers.map((person) => <button type="button" key={person.id} onClick={(event) => { event.stopPropagation(); openWorkspaceMemberProfile(person.id) }} title={`Read by ${person.name}`} aria-label={`Open ${person.name} profile`} className="h-4 w-4 shrink-0 overflow-hidden rounded-full border border-black"><Avatar src={person.avatarSrc} name={person.name} className="h-full w-full" /></button>) : null}</span>
