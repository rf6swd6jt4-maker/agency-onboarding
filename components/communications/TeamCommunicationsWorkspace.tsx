...
function NativeAttachment({ attachment }: { attachment: CommunicationAttachment }) {
    if (attachment.kind === "image") return <a href={attachment.url} target="_blank" rel="noreferrer" className="mb-2 block overflow-hidden rounded-xl bg-black/10"><Image unoptimized src={attachment.url} alt={attachment.fileName} width={800} height={600} className="max-h-80 h-auto w-full object-contain" /></a>
...
function NativeReactionTray({ current, onReply, onReact, side }: { current: string | null; onReply: () => void; onReact: (emoji: string) => void; side: "left" | "right" }) {
    ...
    return <div className="relative flex items-center rounded-full border border-neutral-800 bg-neutral-950 p-1 shadow-xl">
        <button type="button" onClick={onReply} aria-label="Reply" className="flex h-8 w-8 items-center justify-center rounded-full text-neutral-400 hover:bg-neutral-800 hover:text-white">↩</button>
    ...
    useEffect(() => {
        ...
        void supabase.auth.getSession().then(async ({ data }) => {
            ...
            channel = supabase.channel(`communications:${bootstrap.workspaceSlug}`, { config: { private: true } })
                .on("postgres_changes", { event: "*", schema: "public", table: "workspace_native_messages", filter: `workspace_id=eq.${bootstrap.workspaceId}` }, (payload) => { const message = realtimeMessage(payload.new); if (message) updateConversationMessages(message.conversationId, [message]) })
