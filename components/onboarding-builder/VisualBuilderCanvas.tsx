...
import { createButtonBlock, createEstimateBlock, createFormBlock, createVideoBlock, stepEstimate } from "@/lib/onboarding/block-definition"
...
export function VisualBuilderCanvas({
    ...
    function dropBlock(event: DragEvent<HTMLDivElement>, targetId: string) {
        ...
        if (step.blocks[sourceIndex]?.kind === "estimate" && event.shiftKey) return
    ...
    function insertBlock(index: number, kind: "estimate" | "form" | "video" | "button") {
        if (kind === "form" && (target.kind === "bookend" || step.blocks.some((block) => block.kind === "form"))) return
        if (kind === "estimate" && step.blocks.some((block) => block.kind === "estimate")) return
        const block = kind === "estimate" ? createEstimateBlock() : kind === "form" ? createFormBlock() : kind === "video" ? createVideoBlock() : createButtonBlock()
        const blocks = [...step.blocks]
