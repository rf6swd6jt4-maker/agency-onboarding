...
function staticNavigationResults(workspace: { name: string; slug: string }, query: string, role: WorkspaceRole): SearchResult[] {
    ...
    const entries = [
        ...
        ...(canAccessPrivatePanels ? [
            ...
            { id: "settings-client-portal-domain", type: "Settings", label: "Client Portal Domain", description: "Verified client portal hostname", href: workspaceHref(workspace.slug, "settings#client-portal-domain"), path: `${settingsPath} > Connections > Client Portal Domain`, keywords: ["custom domain", "hostname", "portal"] },
