...
export async function POST(request: Request, context: { params: Promise<{ workspaceSlug: string }> }) {
    ...
    if (current?.last_read_at && current.last_read_at >= message.created_at) return Response.json({ cursor: { conversationId, userId: user.id, lastReadMessageId: current.last_read_message_id, lastReadAt: current.last_read_at } })
    ...
    return Response.json({ cursor: { conversationId, userId: user.id, lastReadMessageId: messageId, lastReadAt } })
}
