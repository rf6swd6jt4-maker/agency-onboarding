...
export function RelationshipGantt({ workspaceSlug, relationshipId, plan: initialPlan, canEdit, currentWork, onInvoiceRequest }: {
        ...
        >
            ...
            >
                ...
                <div data-gantt-measure={openEnded ? item.id : undefined} data-gantt-extra={openEnded ? linkSize + 16 : undefined} className={`relative flex items-center gap-1.5 ${openEnded ? "w-max shrink-0 whitespace-nowrap" : "min-w-0 flex-1"}`}>
                    ...
                    {showAssignee && item.assignees[0] ? <div className="relative flex shrink-0 items-center gap-1"><Assignee userId={item.assignees[0].userId} name={item.assignees[0].username} avatarSrc={item.assignees[0].avatarUrl} compact compactSize={row.depth === 0 ? "md" : "sm"} />{item.assignees.length > 1 ? <span className={`shrink-0 font-medium ${row.depth === 0 ? "text-xs" : "text-[9px]"}`}>+{item.assignees.length - 1}</span> : null}</div> : null}
    ...
    return <section id="plan" className="relative isolate mt-4 overflow-hidden rounded-xl border border-neutral-700 bg-neutral-900/70">
        ...
        {currentWork && currentWork.action !== "await_payment" ? <div className="flex flex-wrap items-center gap-x-3 gap-y-2 border-t border-neutral-700 bg-neutral-950 px-3 py-2 text-xs">
            ...
            {currentWork.action !== "sell_client" ? <label className="flex items-center gap-1.5 text-neutral-400"><input type="checkbox" checked={confirmBeforeProceeding} onChange={(event) => { setConfirmBeforeProceeding(event.target.checked); window.localStorage.setItem("betelgeze-current-work-confirm", String(event.target.checked)) }} />Confirm before continuing</label> : null}
            <button type="button" disabled={pending || currentWork.blocked || currentWork.action === "await_payment" || currentWork.action === "await_onboarding"} onClick={() => {
                if (currentWork.action === "sell_client" && onInvoiceRequest) {
                    onInvoiceRequest()
                    return
                }
