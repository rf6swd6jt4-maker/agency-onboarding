...
export function RelationshipGantt({ workspaceSlug, relationshipId, plan: initialPlan, canEdit, currentWork, onInvoiceRequest }: {
    ...
    return <section id="plan" className="relative isolate mt-4 overflow-hidden rounded-xl border border-neutral-700 bg-neutral-900/70">
        ...
        {currentWork && currentWork.action !== "await_payment" ? <div className="flex flex-wrap items-center gap-x-3 gap-y-2 border-t border-neutral-700 bg-neutral-950 px-3 py-2 text-xs">
            ...
            {!(["send_invoice", "sell_client"].includes(currentWork.action ?? "")) ? <label className="flex items-center gap-1.5 text-neutral-400"><input type="checkbox" checked={confirmBeforeProceeding} onChange={(event) => { setConfirmBeforeProceeding(event.target.checked); window.localStorage.setItem("betelgeze-current-work-confirm", String(event.target.checked)) }} />Confirm before continuing</label> : null}
            <button type="button" disabled={pending || currentWork.blocked || currentWork.action === "await_payment" || currentWork.action === "await_onboarding"} onClick={() => {
                if (["send_invoice", "sell_client"].includes(currentWork.action ?? "") && onInvoiceRequest) {
                    onInvoiceRequest()
                    return
                }
            ...
            }} className="h-8 rounded bg-white px-3 text-xs font-semibold text-neutral-950 disabled:opacity-45">{currentWork.action === "sell_client" ? "Sell client" : currentWork.action === "send_invoice" ? "Send invoice" : currentWork.action === "await_payment" ? "Payment pending" : currentWork.action === "await_onboarding" ? "Onboarding in progress" : "Mark complete"}</button>
        </div> : null}
