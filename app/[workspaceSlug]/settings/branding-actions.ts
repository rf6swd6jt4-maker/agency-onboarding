...
export async function saveAgencyBranding(slug: string, swatches: OnboardingBrandSwatch[], assignments: Record<OnboardingThemeSlot, string>): Promise<ConfigurationActionResult<{ workspace_id: string; updated_at: string }>> {
    try {
        ...
        const outcome = await configurationRpc<{ workspace_id: string; updated_at: string }>("save_onboarding_branding", {
